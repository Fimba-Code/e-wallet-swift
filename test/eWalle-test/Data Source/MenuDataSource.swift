...
class MenuDataSource: NSObject, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.sectionName = data[indexPath.row]
...
//        Selects the first row in case nothing was selected yet
        if indexPath.row == 0 {
            cell.selectionLine.isHidden = false
            cell.section.font = UIFont(name: "AvenirNext-DemiBold", size: 16.0)
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        _ = tableViewIndexPaths.map { currentIndexPath in
            ...
            if currentIndexPath == indexPath {
                ...
                if indexPath.row == 0 { showHomeScreen!(true)}
            } else {
                let cell = tableView.cellForRow(at: currentIndexPath) as! MenuCell
                cell.selectionLine.isHidden = true
                cell.section.font = UIFont(name: "AvenirNext-Medium", size: 16.0)
            }
        }
    }
}
